...
def marked_words(df_marked, df_unmarked, target_val, target_col, unmarked_val,occupation_list=None,verbose=False):
    ...
    # print(df)
    # print('hello', df.drop(df[df.occupation!= occupation_list[0]].index))
    # print(df.drop['occupation' != occupation_list[0]])
    if occupation_list:
        # print('occupation', occupation_list)
        for _occupation in occupation_list: # currently does not support multiple occupations
            df_unmarked = df_unmarked.drop(df_unmarked[df_unmarked.occupation != _occupation].index)
            df_marked = df_marked.drop(df_marked[df_marked.occupation != _occupation].index)
    # print(df)
    subdf = df_marked.copy()
    ...
    for i in range(len(unmarked_val)):
        delt = get_log_odds(subdf['text'], df_marked.loc[df_marked[target_col[i]]==unmarked_val[i]]['text'],  df_unmarked['text'], verbose)
        # delt = get_log_odds(subdf['text'],  df_unmarked['text'], df_marked['text'], verbose)
...
def main():
    ...
    # parser.add_argument("unmarked_filename")
    ...
    # unmarked_filename = args.unmarked_filename
    ...
    top_words = marked_words(df, unmarked_df, target_val, target_col, unmarked_val,occupation_list=args.occupation,verbose=args.verbose)
    print("Top words:")
    print(top_words)
